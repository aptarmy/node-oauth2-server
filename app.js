...
const oauth2Middleware = require('./middlewares/oauth2');
...
app.use((req, res, next) => {
  if (['/api/upload', '/oauth2/login'].indexOf(req.path !== -1)) {
    next();
  } else {
 ...
 * OAuth2 routes
 */
app.post('/oauth2/login', oauth2Controller.postOauth2Login(oauth2Server, Oauth2Request, Oauth2Response));
app.get('/oauth2/protected-resources', oauth2Middleware.isAuthenticated(oauth2Server, Oauth2Request, Oauth2Response), oauth2Controller.getProtectedResources);
